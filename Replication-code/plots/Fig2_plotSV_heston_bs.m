function Fig2_plotSV_heston_bs(vimp_LSV, vimp_model, par, grid)
    % Replicates Figure 2: Comparison of performance to replicate the prices from Heston vs Black-Scholes
    ...
    for row = 1:n_slices
        for col = 1:3
            ...
            hold on;
            if row == 1
                plot(grid.kgrid_train, vimp_model(idx_train{col}, :, 1), 'DisplayName', 'True Model');
            else
                plot(grid.kgrid_train, vimp_model(idx_train{col}, :, 2), 'DisplayName', 'True Model');
            end
            hold off;
            if row == 1
            ...
            if col == 1
                if row == 1
                ylabel(sprintf("Heston \nImplied vol.")); % Label rows
                elseif row == 2
                    ylabel(sprintf("Black Scholes \nImplied vol.")); 
                end
    ...
    % annotation('textbox', [0, 0.02, 0, 0.01], 'String', 'Figure 2: Comparison of performance to replicate the prices from Heston vs Black-Scholes', ...
