function Fig4_plotSV_bounds(vimp_LSV, vimp_model, par, grid)
% Replicates Figure 4: Point wise Lower and Upper bounds with GPR
...
for row = 1:n_slices
    for col = 1:3
        ...
        if col == 1
            if row == 1
            ylabel(sprintf("Lower bound \nImplied vol.")); % Label rows
            elseif row == 2
                ylabel(sprintf("Model \nImplied vol.")); 
            elseif row == 3
                ylabel(sprintf("Upper bound \nImplied vol.")); 
            end
